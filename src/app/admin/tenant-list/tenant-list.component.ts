...
export class TenantListComponent {
  ...
  @Input() idBailleur!: number;
  @Input() refPropriete!: string;
  ...
  getOneBailleur() {
    //   alert(this.idBailleur)
    this.bailleurService.onebailleur(this.idBailleur).subscribe(ret => {
      console.log("eeerrerreerrrerrerrrrertt" + ret.data.bailleurId);
      ...
      this.allProprieteBailleurDisponible(ret.data.bailleurId)
  ...
  oneLocataireByRef(locataireRef: string) {
    ...
    this.locataireService.getOneByReference(locataireRef).subscribe(ret => {
      ...
      this.formGroup.controls['locataireDateentre'].setValue(this.oneLocataire.locataireDateentre.substring(0, 10));
  ...
  submitModifLocataire(f: any) {
    var body = {
      ...
      locataireDateentre: f.locataireDateentre + 'T00:00:00.000Z',
  ...
  onChangeURL(urlqr: SafeUrl, parent: any) {
